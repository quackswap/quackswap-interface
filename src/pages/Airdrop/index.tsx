...
export default function Vote() {
  ...
  const uniAmount = useTokenBalance(account ? account : undefined, chainId ? UNI[chainId] : UNI[ChainId.BITTORRENT_MAINNET])
  const sushiAmount = useTokenBalance(
    account ? account : undefined,
    chainId ? SUSHI[chainId] : SUSHI[ChainId.BITTORRENT_MAINNET]
  )
  ...
  const getCard = () => {
    ...
    return (
      <ButtonError error={!!error} padding="16px 16px" width="100%" mt="1rem" onClick={onClaim}>
        {error
          ? error['data']['message']
          : t('airdrop.claim') + claimAmount?.toFixed(0, { groupSeparator: ',' }) + ' QUACK'}
      </ButtonError>
    )
  }
