...
export default function Migrate({
  ...
  const canZap =
    (pglFrom?.involvesToken(QUACK[chainId]) && pglTo?.involvesToken(QUACK[chainId])) ||
    (pglFrom?.involvesToken(WBTT[chainId]) && pglTo?.involvesToken(WBTT[chainId]))
  ...
  const addTokenButton = (token: Token | undefined) => {
    if (!token) return
    if (token.equals(QUACK[chainId])) return
    if (token.equals(WBTT[chainId])) return
    return (
      <ButtonPrimary
        width={'250'}
        onClick={() => {
                  ...
                  <>
                    ...
                    {canZap ? (
                      ...
                      <ErrorText severity={2}>
                        ...
                        <ExternalLink href={'https://bridge.btt.network/convert'}>
                          https://bridge.btt.network/convert
                        </ExternalLink>
                      </ErrorText>
                    )}
                  </>
