...
import { CBTT, currencyEquals, Percent, WBTT } from '@hotcrosscom/quackswap-sdk'
...
export default function RemoveLiquidityModal({
  ...
  async function onRemove() {
    ...
    const currencyBIsBTT = currencyB === CBTT[chainId]
    const oneCurrencyIsBTT = currencyA === CBTT[chainId] || currencyBIsBTT
    ...
    if (approval === ApprovalState.APPROVED) {
      // removeLiquidityBTT
      if (oneCurrencyIsBTT) {
        methodNames = ['removeLiquidityBTT', 'removeLiquidityBTTSupportingFeeOnTransferTokens']
        args = [
          currencyBIsBTT ? tokenA.address : tokenB.address,
          liquidityAmount.raw.toString(),
          amountsMin[currencyBIsBTT ? Field.CURRENCY_A : Field.CURRENCY_B].toString(),
          amountsMin[currencyBIsBTT ? Field.CURRENCY_B : Field.CURRENCY_A].toString(),
          account,
    ...
    // we have a signature, use permit versions of remove liquidity
      ...
      // removeLiquidityBTTWithPermit
      if (oneCurrencyIsBTT) {
        methodNames = ['removeLiquidityBTTWithPermit', 'removeLiquidityBTTWithPermitSupportingFeeOnTransferTokens']
        args = [
          currencyBIsBTT ? tokenA.address : tokenB.address,
          liquidityAmount.raw.toString(),
          amountsMin[currencyBIsBTT ? Field.CURRENCY_A : Field.CURRENCY_B].toString(),
      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            {/*TODO: Translate using i18n*/}
            {'QSL ' + currencyA?.symbol + '/' + currencyB?.symbol} Burned
          </Text>
          <RowFixed>
  ...
  const oneCurrencyIsBTT = chainId && (currencyA === CBTT[chainId] || currencyB === CBTT[chainId])
  const oneCurrencyIsWBTT = Boolean(
    chainId &&
      ((currencyA && currencyEquals(WBTT[chainId], currencyA)) ||
        (currencyB && currencyEquals(WBTT[chainId], currencyB)))
  )
            ...
            <>
              ...
              <LightCard>
                <AutoColumn gap="10px">
                  ...
                  {chainId && (oneCurrencyIsWBTT || oneCurrencyIsBTT) ? (
                    <RowBetween style={{ justifyContent: 'flex-end' }}>
                      {oneCurrencyIsBTT ? (
                        <LinkStyledButton
                          onClick={() => {
                            setCurrencyIdA(currencyA === CBTT[chainId] ? WBTT[chainId].address : currencyIdA)
                            setCurrencyIdB(currencyB === CBTT[chainId] ? WBTT[chainId].address : currencyIdB)
                          }}
                        >
                          {t('removeLiquidity.receiveWbtt', { symbol: NETWORK_WRAPPED_CURRENCY[chainId] })}
                        </LinkStyledButton>
                      ) : oneCurrencyIsWBTT ? (
                        <LinkStyledButton
                          onClick={() => {
                            setCurrencyIdA(
                              currencyA && currencyEquals(currencyA, WBTT[chainId]) ? 'BTT' : currencyIdA
                            )
                            setCurrencyIdB(
                              currencyB && currencyEquals(currencyB, WBTT[chainId]) ? 'BTT' : currencyIdB
                            )
                          }}
                        >
                          {t('removeLiquidity.receiveBtt', { symbol: NETWORK_CURRENCY[chainId] })}
                        </LinkStyledButton>
                      ) : null}
                    </RowBetween>
                  ) : null}
