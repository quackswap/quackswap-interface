...
import { CBTT, Token, CHAINS } from '@hotcrosscom/quackswap-sdk'
...
export default function DoubleSidePoolCard({
  ...
  const token: Token =
    currency0 === CBTT[chainId] || currency1 === CBTT[chainId]
      ? currency0 === CBTT[chainId]
        ? token1
        : token0
      : token0.equals(QUACK[token0.chainId])
      ? token1
      : token0
      ...
      <TopSection isStaking={isStaking}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          {(isStaking || !stakingInfo.isPeriodFinished) && (
            <StyledInternalLink
              to={`/quack/${currencyId(currency0, chainId)}/${currencyId(currency1, chainId)}/${version}`}
              style={{ width: '100%' }}
            >
              <ButtonPrimary padding="8px" borderRadius="8px">
          ...
          <Box>
            <BottomSection showBackground={true}>
              ...
              <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
                ...
                {`${stakingInfo.rewardRatePerWeek?.toSignificant(4, { groupSeparator: ',' })} QUACK / week`}
              </TYPE.black>
            </BottomSection>
