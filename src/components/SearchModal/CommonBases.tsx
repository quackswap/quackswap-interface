  ...
  return (
    <AutoColumn gap="md">
      ...
      <AutoRow gap="4px">
        <BaseWrapper
          onClick={() => {
            if (!selectedCurrency || !currencyEquals(selectedCurrency, CBTT[chainId])) {
              onSelect(CBTT[chainId])
            }
          }}
          disable={selectedCurrency === CBTT[chainId]}
        >
          {chainId && (
            <CurrencyLogo currency={chainId && CBTT[chainId]} style={{ marginRight: 8 }} chainId={chainId} />
          )}
