  ...
  const getBalance = async () => {
    if (account && chainId) {
      ...
      const requestTokens: TokenDataUser[] = data.map((token: any) => {
        if (token?.id?.toLowerCase() === 'btt') {
          return new TokenDataUser(CBTT[chainId], token?.price, token?.amount)
        }
      ...
      if (chainId === ChainId.BITTORRENT_MAINNET) {
