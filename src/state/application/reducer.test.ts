...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BITTORRENT_MAINNET, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.BITTORRENT_MAINNET]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BITTORRENT_MAINNET, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.BITTORRENT_MAINNET]).toEqual(3)
    })
    it('works with non-set chains', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.FUJI, blockNumber: 2 }))
