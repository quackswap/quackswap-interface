...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      store.dispatch(
        addTransaction({
          chainId: ChainId.BITTORRENT_MAINNET,
          summary: 'hello world',
          hash: '0x0',
      ...
      expect(txs[ChainId.BITTORRENT_MAINNET]).toBeTruthy()
      expect(txs[ChainId.BITTORRENT_MAINNET]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.BITTORRENT_MAINNET]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BITTORRENT_MAINNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BITTORRENT_MAINNET] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.BITTORRENT_MAINNET }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BITTORRENT_MAINNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BITTORRENT_MAINNET] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
