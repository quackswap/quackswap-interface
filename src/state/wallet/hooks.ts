...
export function useCurrencyBalances(
  ...
  const containsETH: boolean = useMemo(
    () => currencies?.some(currency => chainId && currency === CBTT[chainId]) ?? false,
    [chainId, currencies]
  )
  ...
  return useMemo(
    () =>
      currencies?.map(currency => {
        ...
        if (chainId && currency === CBTT[chainId]) return ethBalance[account]
        return undefined
      }) ?? [],
...
// get the total owned and unharvested QUACK for account
