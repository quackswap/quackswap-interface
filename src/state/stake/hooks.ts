...
import { DAIe, QUACK, USDC, USDCe, USDTe, axlUST } from '../../constants/tokens'
...
export interface SingleSideStakingInfo extends StakingInfoBase {
  ...
  rewardToken: Token
  // total staked QUACK in the pool
...
export interface DoubleSideStakingInfo extends StakingInfoBase {
  ...
  // the pool weight
  multiplier: JSBI
  // total staked BTT in the pool
  totalStakedInWbtt: TokenAmount
...
const calculateTotalStakedAmountInBttFromPng = function(
  amountStaked: JSBI,
  amountAvailable: JSBI,
  bttPngPairReserveOfPng: JSBI,
  bttPngPairReserveOfWbtt: JSBI,
  reserveInPng: JSBI,
  chainId: ChainId
): TokenAmount {
  if (JSBI.EQ(amountAvailable, JSBI.BigInt(0))) {
    return new TokenAmount(WBTT[chainId], JSBI.BigInt(0))
  }

  const oneToken = JSBI.BigInt(1000000000000000000)
  const bttPngRatio = JSBI.divide(JSBI.multiply(oneToken, bttPngPairReserveOfWbtt), bttPngPairReserveOfPng)
  const valueOfPngInBtt = JSBI.divide(JSBI.multiply(reserveInPng, bttPngRatio), oneToken)

  return new TokenAmount(
    WBTT[chainId],
    JSBI.divide(
      JSBI.multiply(
        JSBI.multiply(amountStaked, valueOfPngInBtt),
        JSBI.BigInt(2) // this is b/c the value of LP shares are ~double the value of the wbtt they entitle owner to
      ),
      amountAvailable
    )
  )
...
const calculateTotalStakedAmountInBtt = function(
  amountStaked: JSBI,
  amountAvailable: JSBI,
  reserveInWbtt: JSBI,
  chainId: ChainId
  ...
  if (JSBI.GT(amountAvailable, 0)) {
    // take the total amount of LP tokens staked, multiply by BTT value of all LP tokens, divide by all LP tokens
    return new TokenAmount(
      WBTT[chainId],
      JSBI.divide(
        JSBI.multiply(
          JSBI.multiply(amountStaked, reserveInWbtt),
          JSBI.BigInt(2) // this is b/c the value of LP shares are ~double the value of the wbtt they entitle owner to
        ),
        amountAvailable
      )
    ...
    return rewardsAddresses.reduce<DoubleSideStakingInfo[]>((memo, rewardsAddress, index) => {
      ...
      if (
        ...
        if (
          ...
          bttPngPairState === PairState.INVALID ||
          bttPngPairState === PairState.NOT_EXISTS
        ) {
        ...
        const wbtt = tokens[0].equals(WBTT[tokens[0].chainId]) ? tokens[0] : tokens[1]
        ...
        const isBttPool = tokens[0].equals(WBTT[tokens[0].chainId])
        const totalStakedInWbtt = isBttPool
          ? calculateTotalStakedAmountInBtt(
              totalSupplyStaked,
              totalSupplyAvailable,
              pair.reserveOf(wbtt).raw,
              chainId
            )
        ...
        const totalStakedInUsd = totalStakedInWbtt && (usdPrice?.quote(totalStakedInWbtt, chainId) as TokenAmount)
        ...
        memo.push({
          ...
          totalStakedInWbtt: totalStakedInWbtt,
...
export function useSingleSideStakingInfo(
  ...
  const getAmountsOutArgs = useMemo(() => {
    const amountIn = '1' + '0'.repeat(18) // 1 QUACK
...
export function useGetPairDataFromPair(pair: Pair) {
  ...
  const dummyToken = new Token(chainId, ZERO_ADDRESS, 18, 'QUACK', 'Pangolin')
...
export const tokenComparator = (
  ...
  // Sort BTT last
  if (addressA === WBTT[ChainId.BITTORRENT_MAINNET].address) return 1
  else if (addressB === WBTT[ChainId.BITTORRENT_MAINNET].address) return -1
  // Sort QUACK first
  else if (addressA === QUACK[ChainId.BITTORRENT_MAINNET].address) return -1
  else if (addressB === QUACK[ChainId.BITTORRENT_MAINNET].address) return 1
  // Sort axlUST first
  else if (addressA === axlUST[ChainId.BITTORRENT_MAINNET].address) return -1
  else if (addressB === axlUST[ChainId.BITTORRENT_MAINNET].address) return 1
  // Sort USDC first
  else if (addressA === USDC[ChainId.BITTORRENT_MAINNET].address) return -1
  else if (addressB === USDC[ChainId.BITTORRENT_MAINNET].address) return 1
  // Sort USDCe first
  else if (addressA === USDCe[ChainId.BITTORRENT_MAINNET].address) return -1
  else if (addressB === USDCe[ChainId.BITTORRENT_MAINNET].address) return 1
  else return 0
}
  ...
  const arr = useMemo(() => {
    ...
    return pairAddresses.reduce<any[]>((memo, _pairAddress, index) => {
      ...
      if (
        ...
        const isPngPool = pair.involvesToken(QUACK[chainId])

        let totalStakedInUsd = CHAINS[chainId || ChainId].mainnet
          ? new TokenAmount(DAIe[chainId], BIG_INT_ZERO)
          : undefined
        const totalStakedInWbtt = new TokenAmount(WBTT[chainId], BIG_INT_ZERO)
        ...
        if (JSBI.equal(totalSupplyAvailable, BIG_INT_ZERO)) {
          ...
          const _totalStakedInWbtt = calculateTotalStakedAmountInBtt(
            totalSupplyStaked,
            totalSupplyAvailable,
            pair.reserveOf(WBTT[chainId]).raw,
            chainId
          )
          totalStakedInUsd = CHAINS[chainId || ChainId].mainnet
          ...
          // Contains no stablecoin, WBTT, nor QUACK
        ...
        stakingContract
          ...
          .then((response: TransactionResponse) => {
            addTransaction(response, {
              summary: t('earnPage.stakeStakingTokens', { symbol: 'QUACK' })
            })
...
export const sortingOnBttStake = (info_a: DoubleSideStakingInfo, info_b: DoubleSideStakingInfo) => {
  // only first has ended
  if (info_a.isPeriodFinished && !info_b.isPeriodFinished) return 1
  // only second has ended
  if (!info_a.isPeriodFinished && info_b.isPeriodFinished) return -1
  // greater stake in btt comes first
