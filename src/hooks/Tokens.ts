...
import { Currency, CBTT, Token, currencyEquals, CHAINS, ChainId } from '@hotcrosscom/quackswap-sdk'
...
export function useCurrency(currencyId: string | undefined): Currency | null | undefined {
  ...
  const token = useToken(isBTT ? undefined : currencyId)
  return isBTT ? chainId && CBTT[chainId] : token
}
...
export function useCoinGeckoTokenData(coin: Token) {
  ...
  useEffect(() => {
    const getCoinData = async () => {
      const chain = coin.chainId === 43113 ? CHAINS[ChainId.BITTORRENT_MAINNET] : CHAINS[coin.chainId]
